...
class ImageDatabase:
    ...
    def create_output(self, add_backslash_before_parenthesis=False, keep_tokens_separator: str= "|||", main_tags: list[str]=[], secondary_tags: list[str]=[], use_aesthetic_score=False, score_trigger=True, use_sentence=False, sentence_in_trigger=False, remove_tags_in_sentence=True, shuffle_tags=True):
        """
        ...
        for segment in self.sentence_description.get_output_info():
            if isinstance(segment, tuple):
                if segment[0] == "RECT":
                    ...
                    result += self.rects[[rect.name for rect in self.rects].index(segment[1])].create_output(add_backslash_before_parenthesis=add_backslash_before_parenthesis, keep_tokens_separator=keep_tokens_separator, main_tags=main_tags, secondary_tags=secondary_tags, use_sentence=use_sentence, shuffle_tags=shuffle_tags)
