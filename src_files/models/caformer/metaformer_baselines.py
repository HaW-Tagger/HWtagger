...
class Attention(nn.Module):
    ...
    def __init__(self, dim, head_dim=32, num_heads=None, qkv_bias=False,
        attn_drop=0., proj_drop=0., proj_bias=False, is_2d=True, **kwargs):
        super().__init__()
        ...
        if is_2d:
            self.re_qkv = Rearrange('b h w (nh dh) -> b nh (h w) dh', nh=self.num_heads)
        else:
        ...
        if use_xformers:
            ...
            self.attention = build_attention(my_config)
        
    def forward(self, q, k=None, v=None, att_mask=None):
        if k is None:
            B,H,W,C = q.shape
            k,v = q,q
        ...
        if use_xformers:
            x = self.attention(q,k,v, att_mask=att_mask)
        else:
            attn = einsum(q * self.scale, k, 'b nh q dh, b nh k dh -> b nh q k').softmax(dim=-1)
            if att_mask:
                attn*=att_mask
            attn = self.attn_drop(attn)
            x = einsum(attn, v, 'b nh q k, b nh k dh -> b nh q dh')

        if self.is_2d:
            x = rearrange(x, 'b nh (h w) dh -> b h w (nh dh)', h=H)
        else:
...
@register_model
def caformer_d30(pretrained=False, **kwargs):
    model = MetaFormer(
        depths=[4, 12, 12, 3],
        dims=[128, 192, 384, 576],
        token_mixers=[SepConv, SepConv, Attention, Attention],
        head_fn=MlpHead,
        **kwargs)
    model.default_cfg = default_cfgs['caformer_d30']
    return model
